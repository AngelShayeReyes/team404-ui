...
router.get('/viewjobspecification/:jobId', async (req, res) => {
    ...
    .then(function (response) {
        // handle success
        res.render('viewjobspecification', { job_name: response.data.title, contract_type: response.data.contractType, job_description: response.data.description, job_locations: response.data.locations, job_capability: response.data.capability, job_band_level: response.data.band})
    })
    .catch(function (error) {
        // handle error
        console.log(error);
